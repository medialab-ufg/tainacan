...
include_once ( dirname(__FILE__) . '/../../helpers/config/extras_helper.php');
include_once ( dirname(__FILE__) . '/../../views/event/js/list_js.php');

$col_id = get_option('collection_root_id');
$extrasHelper = new ExrasHelper();
$evts = $extrasHelper->list_events( ["collection_id" => $col_id, "only_data" => true]);

if( is_array($evts) && !empty($evts) ) {
    if( array_key_exists("events_observed", $evts) )
        $events_observed = $evts["events_observed"];

    if( array_key_exists("events_not_observed", $evts) )
        $events_not_observed = $evts["events_not_observed"];

    if( array_key_exists("moderation_type", $evts) )
        $moderation_type = $evts["moderation_type"];
}
?>
<div id="events_title" class="col-md-12">
    <div class="col-md-12 config_default_style" id="events_settings">
        <?php ViewHelper::render_config_title( _t("Events") ); ?>
                            ...
                            <?php if (isset($events_not_observed)) {     // se existir eventos a serem verificaos   ?>
                                <table id="event_not_verified_table" class="table table-striped table-bordered" cellspacing="0" width="100%">  <!--class="table table-bordered" style="background-color: #d9edf7;"-->
                                    ...
                                    <tr>
                                        ...
                                        <?php if ($moderation_type == 'democratico' && (current_user_can('manage_options') || verify_collection_moderators($col_id, get_current_user_id()))): ?>
                                            <th>
                                    ...
                                    <?php foreach ($events_not_observed as $event) { ?>
                                        <tr>
                                            ...
                                            <td>
                                                <?php if ((current_user_can('manage_options') || verify_collection_moderators($col_id, get_current_user_id()))): ?>
                                                    <a style="cursor:pointer;" onclick="show_verify_event_not_confirmed('<?= $event['id'] ?>', '<?= $col_id ?>')"><span class="glyphicon glyphicon-eye-open"></span>&nbsp;<?php echo $event['name']; ?></a>
                                                <?php else: ?>
                                                    <a style="cursor:pointer;" onclick="show_unconfirmed_users_events('<?= $event['id'] ?>', '<?= $col_id ?>')"><span class="glyphicon glyphicon-eye-open"></span>&nbsp;<?php echo $event['name']; ?></a>
                                                <?php endif; ?>
                                            </td>
                                            <?php if ($moderation_type == 'moderador' || !isset($moderation_type) || empty($moderation_type)) { ?>
                                                <td>
                                                    <a style="cursor:pointer;" onclick="show_verify_event_not_confirmed('<?= $event['id'] ?>', '<?= $col_id ?>')"><span class="glyphicon glyphicon-eye-open"></span>&nbsp;<?php _e('Not verified', 'tainacan') ?></a>
                                                </td>
                                            <?php } else { ?>
                                                <td>
                                ...
                                <table id="event_verified_table"  class="table table-striped table-bordered" >
                                    ...
                                    <?php foreach ($events_observed as $event) { ?>
                                        <tr>
                                            ...
                                            <?php if ($moderation_type == 'moderador' || !isset($moderation_type) || empty($moderation_type)) { ?>
                                                <td>
                                                    <a style="cursor:pointer;" onclick="show_verify_event_confirmed('<?= $event['id'] ?>', '<?= $col_id ?>')"><span class="glyphicon glyphicon-eye-open"></span>&nbsp;
                                                        <?php
