...
class EventPropertyTermDelete extends EventModel {
    ...
    public function generate_title($data) {
        ...
        $title = __('Delete the term property ','tainacan').' ( <i>'.$property->name.'</i> ) '.__(' in the collection ','tainacan').' '.' <b><a target="_blank" href="'.  get_the_permalink($collection->ID).'">'.$collection->post_title.'</a></b> ';
        return $title;
    }
