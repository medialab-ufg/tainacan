...
class EventPropertyDataDelete extends EventModel {
    ...
    public function generate_title($data) {
        ...
        $property = get_term_by('id',$data['socialdb_event_property_data_delete_id'],'socialdb_property_type');
        $title = __('Delete the data property ','tainacan').' ( <i>'.$property->name.'</i> ) '.__(' in the collection ','tainacan').' '.' <b><a target="_blank" href="'.  get_the_permalink($collection->ID).'">'.$collection->post_title.'</a></b> ';;
        return $title;
    }
    ...
    public function delete_property($event_id,$data,$automatically_verified) {
        ...
        $data['property_delete_id'] = get_post_meta($event_id, 'socialdb_event_property_data_delete_id',true) ;
        $data['property_category_id'] = get_post_meta($event_id, 'socialdb_event_property_data_delete_category_root_id',true) ;
        ...
        if(!$categories_used||
            ...
            if(isset($verify->term_id)){
                do_action('after_event_delete_property_data',$verify,$event_id);
            }
        }else{
        ...
        if ($verify){
            ...
            $data['msg'] = __('The event was successful');
            $data['type'] = 'success';
             $data['title'] = __('Success','tainacan');
        } else {
