...
class EventTermDelete extends EventModel {
    ...
    public function generate_title($data) {
        ...
        $title = __('Delete the category ','tainacan') . '( <i>'.$category->name.'</i> )' . __(' in the collection ','tainacan') .' '.' <b><a href="'.  get_the_permalink($collection->ID).'">'.$collection->post_title.'</a></b> ';
        return $title;
    }
