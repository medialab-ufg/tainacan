...
class EventTagDelete extends EventModel {
    ...
    public function generate_title($data) {
        ...
        $title = __('Delete the tag ','tainacan').'( <b>'.$category->name.'</b> )'.__(' in the collection ','tainacan').' <b><a target="_blank" href="'.  get_the_permalink($collection->ID).'">'.$collection->post_title.'</a></b> ';
        return $title;
    }
