...
include_once ('js/edit_data_property_form_js.php'); ?>
  <!-- TAINACAN: formulario para edicao de propiedade de atributo  -->
<h4><?php echo __('Editing the property: ','tainacan').$value->name; ?></h4>
<form  id="submit_form_edit_property_data">
    ...
    <div class="create_form-group">
        ...
        <input type="text" class="form-control" id="event_edit_property_data_name" name="socialdb_event_property_data_edit_name" required="required" value="<?php echo $value->name; ?>" placeholder="<?php _e('Property Data name','tainacan'); ?>">
    </div>
    <div class="form-group">
        <label for="event_edit_property_data_widget"><?php _e('Property data widget','tainacan'); ?></label>
        <select class="form-control" id="event_edit_property_data_widget" name="socialdb_event_property_data_edit_widget">
            ...
            <option <?php if($value->type=='date') echo 'selected="selected";' ?> value="date"><?php _e('Date','tainacan'); ?></option>
             <option <?php if($value->type=='numeric') echo 'selected="selected";' ?> value="date"><?php _e('Numeric','tainacan'); ?></option>
              <option <?php if($value->type=='autoincrement') echo 'selected="selected";' ?> value="date"><?php _e('Auto-Increment','tainacan'); ?></option>
        </select>
    </div>
    <div class="form-group">
        <label for="event_edit_property_data_column_ordenation"><?php _e('Property data column ordenation','tainacan'); ?></label>
        <input type="radio" <?php if($value->metas->socialdb_property_data_column_ordenation=='true') echo 'checked="checked";' ?> name="socialdb_event_property_data_edit_ordenation_column" id="event_edit_property_data_column_ordenation_true" value="true">&nbsp;<?php _e('Yes','tainacan'); ?>
        <input type="radio" <?php if($value->metas->socialdb_property_data_column_ordenation=='false') echo 'checked="checked";' ?> name="socialdb_event_property_data_edit_ordenation_column" id="event_edit_property_data_column_ordenation_false" value="false">&nbsp;<?php _e('No','tainacan'); ?>
    </div>
    <div class="form-group">
        <label for="event_edit_property_data_required"><?php _e('Property data required','tainacan'); ?></label>
        <input type="radio" <?php if($value->metas->socialdb_property_required=='true') echo 'checked="checked";' ?> name="socialdb_event_property_data_edit_required" id="event_edit_property_data_required_true" value="true">&nbsp;<?php _e('Yes','tainacan'); ?>
        <input type="radio" <?php if($value->metas->socialdb_property_required=='false') echo 'checked="checked";' ?> name="socialdb_event_property_data_edit_required" id="event_edit_property_data_required_false"  value="false">&nbsp;<?php _e('No','tainacan'); ?>
    </div>
    <input type="hidden" id="event_edit_property_data_collection_id" name="socialdb_event_collection_id" value="<?php echo $collection_id; ?>">
