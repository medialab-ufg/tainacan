<?php if (get_option('collection_root_id') != $collection_id): ?>
    ...
    <?php if ($is_moderator || get_post($curr_id)->post_author == get_current_user_id()): ?>
        <li>
            <a onclick="delete_permanently_object('<?= __('Delete Object', 'tainacan') ?>', '<?= __('Are you sure to remove the object permanently: ', 'tainacan') . get_the_title() ?>', '<?php echo $curr_id ?>')" style="cursor: pointer;" class="remove">
    ...
    if ($is_moderator || get_post(get_the_ID())->post_author == get_current_user_id()): ?>
        <li>
            ...
            <a onclick="delete_permanently_object('<?= __('Delete Object', 'tainacan') ?>', '<?= __('Are you sure to remove the collection permanently: ', 'tainacan') . get_the_title() ?>', '<?php echo $curr_id ?>')" href="#" class="remove">
                <span class="glyphicon glyphicon-trash"></span>
            </a>
        </li>
        <li>
