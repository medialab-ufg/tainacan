<script>
    $(function () {
            ...
            paged: function (page) {
                ...
                $.ajax({
                    ...
                    data: {operation: 'wpquery_page_advanced_item', 
                        ...
                        posts_per_page:$('#items-per-page-items').val(),
                        value: page}
        ...
        $('.pagination_collection').jqPagination({
            link_string: '/?page={page_number}',
            max_page: $('#number_pages_collection').val(),
            paged: function (page) {
                $('html,body').animate({scrollTop: 0}, 'slow');
                $.ajax({
                    type: "POST",
                    url: $('#src').val() + "/controllers/advanced_search/advanced_search_controller.php",
                    data: {operation: 'wpquery_page_advanced_collection',posts_per_page:$('#items-per-page-collection').val(), wp_query_args: $('#advanced_search_wp_query_args_collection').val(), value: page}
                }).done(function (result) {
                    elem = jQuery.parseJSON(result);
                    $('#ad_search_collection').html(elem.page);
                    $('#advanced_search_wp_query_args_collection').val(elem.args);
                });
            }
        });
        $('.nav-tabs').tab();
     ...
     //BEGIN: funcao que altera a qtd de items na visualizacao
     function change_qtd_elements_collection(seletor){
        $.ajax({
            type: "POST",
            url: $('#src').val() + "/controllers/advanced_search/advanced_search_controller.php",
            data: {operation: 'wpquery_page_advanced_collection',posts_per_page:$(seletor).val(), 
                wp_query_args: $('#advanced_search_wp_query_args_collection').val(), 
                value: $('#actual_page_collection').val()}
        }).done(function (result) {
            elem = jQuery.parseJSON(result);
            $('#ad_search_collection').html(elem.page);
            $('#advanced_search_wp_query_args_collection').val(elem.args);
