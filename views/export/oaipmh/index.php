<?php

/* 
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
include_once ('js/index_js.php');
?>
<div class="panel-heading" style="max-width: 100%;">
    <div class="row">
        <div class="col-md-12">
            <dl>
                <dt>Example Tables
                <dd><a href="doc/oai_records_mysql.sql">OAI Records (mysql)</a></dd>
                <dd><a href="doc/oai_records_pgsql.sql">OAI Records (pgsql)</a></dd>
                </dt>
                <dt>Query and check your Data-Provider</dt>
                <dd><a on href="<?php echo get_template_directory_uri() ?>/controllers/export/oaipmh_controller.php">Identify</a></dd>
                <dd><a href="#">ListMetadataFormats</a></dd>
                <dd><a href="#">ListSets</a></dd>
                <dd><a href="#">ListIdentifiers</a></dd>
                <dd><a href="<?php echo get_template_directory_uri() ?>/controllers/export/oaipmh_controller.php?operation=listRecords">ListRecords</a></dd>
                <dd><a href="#">GetRecord</a></dd>
                </dt>
            </dl>
        </div>
    </div>
</div>    