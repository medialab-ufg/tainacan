...
<?php if (!empty($home_search_term)) { ?>
                $('#select_between_collections').val('<?php echo get_option('collection_root_id'); ?>');
                //$('#advanced_search_submit').trigger('submit');
<?php } ?>
            $("#select_between_collections").removeAttr('disabled');
            $("#select_between_collections").removeClass('ui-autocomplete-loading');
            $("#select_between_collections").html('');
            var collection_root_id = '<?php echo get_option('collection_root_id'); ?>';
            $("#select_between_collections").append("<option  value='" + collection_root_id + "' selected='selected' ><?php _e('All Colections', 'tainacan') ?></option>");
            $.each(elem, function (idx, collection) {
                if (collection && collection !== false && collection.value != collection_root_id) {
                    $("#select_between_collections").append("<option  value='" + collection.value + "' >" + collection.name + "</option>");
                }
            });
...
//                                }
    ...
    $('.expand-all').toggle(function () {
        ...
        $(this).find("div.action-text").text('<?php _e('Visualization', 'tainacan') ?>');
