...
include_once( dirname( __FILE__ ) . '/lib/class-wp-json-meta-taxonomy.php' );


 /**
 * TAINACAN MODIFICATIONS.
 *
		...
		if (is_array( $data['x-categories'])) {
            for($x = 0; $x < count($data['x-categories']); $x++) {
                if (ctype_digit($data['x-categories'][$x])) {
                    $cat = get_term_by( 'id', $data['x-categories'][$x], 'socialdb_category_type' );
                    if(isset($cat->term_id)){
                        $data['x-categories'][$x] = $cat->term_id;
                    }
                }
            }
            wp_set_object_terms($post['ID'], $data['x-categories'], 'socialdb_category_type');
...
function json_api_default_filters( $server ) {
	...
	add_filter( 'json_prepare_post',   array( $wp_json_taxonomies, 'add_term_data'     ), 10, 3 );
       ...
       // Term meta. Projeto Tainacan
	$wp_json_term_meta = new WP_JSON_Meta_Taxonomy( $server );
	add_filter( 'json_endpoints',    array( $wp_json_term_meta, 'register_routes'    ), 0 );
	add_filter( 'json_prepare_post', array( $wp_json_term_meta, 'add_term_meta_data' ), 10, 3 );
	add_filter( 'json_insert_post',  array( $wp_json_term_meta, 'insert_term_meta'   ), 10, 2 );        
