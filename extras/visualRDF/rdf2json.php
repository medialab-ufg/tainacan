...
function get_name_tainacan($url){
    if(strpos($url, '?category=')!==false){
        $title = get_term_by('slug', explode('?category=',$url)[1], 'socialdb_category_type')->name;
        if($title=='socialdb_category'){
            return __('Category','tainacan');
        }else{
            return $title;
        }
    }else if(strpos($url, '?property=')!==false){
        return get_term_by('slug', explode('?property=',$url)[1], 'socialdb_property_type')->name;
    }else if(strpos($url, '?item=')!==false){
        $the_slug = (explode('?item=',$url)[1]);
        return get_page_by_path($the_slug,OBJECT,'socialdb_object')->post_title;
    }
    return $url;
}
...
if(isset($_GET['url'])){
	...
	foreach($triples as $t){
		...
		if(!isset($nodes[$t['s']])){
                    $nodes[$t['s']] = $c++;
                    $results['nodes'][] = array("name" => get_name_tainacan($s) , "uri" => $t['s'], "type" => $t['s_type']);
		}
		if($t['o_type'] == 'literal'){
			...
			if(!isset($nodes[$t['o']])){
				$nodes[$t['o']] = $c++;
				$results['nodes'][] = array("name" => get_name_tainacan(uri2curie($t['o'])), "uri" => $t['o'], "type" => $t['o_type']);
			}
